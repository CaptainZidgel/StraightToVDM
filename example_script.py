...
for fI, file in d:  #for index, file path in filtered directory
        ...
        L = vdm.EventList(file, d, Filter=vdm.EventFilter)
...
''' PREC
d = vdm.Directory("tf/demos/prec/killstreaks.txt", Type="prec")
for fI, file in d:
    L = vdm.EventList(file, d)
    print(file)
    for eI, event in L:
        print(event['tick'], event['name'])
        vdm.RecordEvent(event, eI, L)
    L.write()
'''
