...
#Only process events that are named Bookmarks (auto-bookmarked events are named 'Killstreak')
...
'''
Create a vdm for this specific file - the reason the parameters here are so... 
'granular' is because this was mostly copy pasted from my original function, which processed every file in a loop.
It is impossible to separate the functionality from the loop it is in, therefore this will only be called either by my loop
or a scripter's loop.
...
'''
def Process(file, collection, fI, START_MARGIN, END_MARGIN, SKIP_MARGIN):
	with open(file, "r") as f:			#Open JSON for reading
		...
		for eI, e in enumerate(events):
			...
			if eI+1 == len(events): #This is the last event in the demo (Not using elif, so I can insert recordings between)
				if fI+1 == len(collection):	#If this is the last file
					action = StopDemo(tick + 1)
					action_stack.append(action())
				else:
					action = NextDem(tick + 1, collection[fI+1])
					action_stack.append(action())
...
'''
 ...
 SKIP_MARGIN - How many ticks before a recording starts to end the skip (tick - START_MARGIN - SKIP_MARGIN)
'''
def ProcessAll(dir, START_MARGIN=500, END_MARGIN=0, SKIP_MARGIN=1):
	bmarks = searchdir(dir, ".json", Prefix=True)
	bmarks = list(filter(HasEvents, bmarks))
	bmarks = list(filter(JSONFilter, bmarks))
	for fI, file in enumerate(bmarks):	#Loop over each json
		Process(file, bmarks, fI, START_MARGIN, END_MARGIN, SKIP_MARGIN)
